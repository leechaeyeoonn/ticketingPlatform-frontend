...
export default function PerformanceDetailPage() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-stone-950 text-indigo-400">
        Loading...
      </div>
    );
  }
        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="flex-1 flex flex-col justify-end pb-4">
            ...
            <div className="flex justify-between items-start mb-6">
              <h1 className="text-4xl font-bold text-white leading-tight">{performance.title}</h1>
              {/* 💖 찜하기 버튼 추가 */}
              <button
                onClick={() => toggleLike(performance.id)}
                className={`p-3 rounded-full transition-all ${isHearted ? 'bg-pink-500/20 text-pink-500' : 'bg-stone-800 text-stone-400 hover:bg-stone-700'}`}
              >
          ...
          <div className="hidden md:block w-80 pt-20">
            <div className="bg-stone-900 border border-stone-800 rounded-xl p-6 shadow-xl sticky top-24">
              ...
              <p className="text-stone-400 text-sm mb-6">
                원하시는 회차를 선택하여 예매를 진행하세요.
              </p>
              <button
                onClick={() =>
                  navigate(`/reservation/${id}`, {
                    state: { title: performance.title, poster: performance.posterUrl },
                  })
                }
